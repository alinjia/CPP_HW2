...
/*Task 7*/
int findLucky(std::vector<int>& arr) { // Функция для поиска счастливого числа
    std::vector<int> frequency(arr.size() + 1, 0); // Инициализируем вектор частот
    // frequency - вектор, хранящий количество вхождений чисел массива
    ...
    for (int num : arr) {
        if (num >= 1 && num <= arr.size()) { // Проверяем, что число входит в нужный диапазон
            frequency[num]++; // Увеличиваем счетчик частоты вхождения числа
        }
    }


    for (int i = arr.size(); i >= 1; i--) {  // Проходим по счетчикам, начиная с конца
        if (frequency[i] == i) { // Если счетчик равен его индексу (числу), то это счастливое число
...
/*Task 8*/
int TheMajorityElement(int *array, int length) { //функция для нахождения элемента, который появляется больше чем в половине массива (по условию такой элемент точно есть)
...
int main()
{
   ...
   //Если операция не соответствует ни одному из знаков операций, выводится сообщение об ошибке и цикл продолжается
              default:
                if (operation!=' ')
                 std::cout << "Invalid value" << std::endl;
                 continue;
                    }
    std::cout << "Result: " << funcPtr(num1, num2) << std::endl;// Выводится результат выполнения операции, вызванной через указатель на функцию
